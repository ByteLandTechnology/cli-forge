...
Use this brief as the planning-stage contract whenever Speckit generates or
reviews `spec.md`, `plan.md`, or `tasks.md` for a `cli-forge` workflow. It is
shorter than the original long-form governance doc and is meant to keep planning
decisions concrete, comparable, and implementation-ready.

## Purpose And Scope

- Plan Rust-based CLI Skills that will be scaffolded, extended, validated, or
  prepared for publish through the `cli-forge` stage skills.
- Lock the user-visible CLI contract before implementation starts.
- Surface risks early enough that Speckit can choose scope, sequencing, and
...
Speckit plans should include the expected generated structure and validation
evidence:
...
- coverage for help, format switching, error handling, Active Context, and any
  enabled `stream` or `repl` behavior
- verification steps such as `cargo build`, `cargo test`,
  `cargo clippy -- -D warnings`, and `cargo fmt --check`

## Speckit Workflow Expectations

- `specify`: define the agent-facing use case, acceptance criteria, and scope
  boundaries.
- `plan`: map user stories to commands, outputs, state handling, and package
  boundaries; include a planning-brief check before implementation starts.
- `tasks`: make contract work first-class and blocking where needed,
  especially `SKILL.md`, CLI help surfaces, and integration tests.
- `implement`: follow the approved plan instead of inventing new contract
  behavior ad hoc.
- `validate`: compare the built artifact against the planned contract, not
